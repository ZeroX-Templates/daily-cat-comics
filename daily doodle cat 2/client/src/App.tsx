...
function App() {
  ...
  const fetchDailyComic = useCallback(async () => {
    setState(s => ({ ...s, status: 'loading', comic: null, error: null }));
    try {
      const response = await fetch('/api/daily-comic');
      
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.message || 'Failed to fetch comic');
      }
  ...
  useEffect(() => {
    ...
    const timerId = setInterval(() => {
      ...
      const currentDate = now.toLocaleDateString('en-US', { timeZone });
